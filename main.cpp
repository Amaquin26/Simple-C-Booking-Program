...
//Global Variable
    ...
    //Sports/Games
    string extremeGames[] = {"Skydiving","Base Jumping","Paragliding","Bungee Jumping","Hand Gliding"
    ,"Wingsuit Flying","Bodyflight","Sky Surfing","Ultralight Aviation"};
            ...
            else
            {
                for(int j=i+1;j < validUsersSize; j++)
                {
                    if(validUsers[i] != userName)
                    {
                        ...
                        if(j == validUsersSize - 1)
                        {
                            cout << "\nSorry account doesn't exist" << endl;
                            IDfound = false;
                            j =0;
                            i = 0;
                            //RelogIn
